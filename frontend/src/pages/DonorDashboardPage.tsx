...
export const DonorDashboardPage: React.FC = () => {
  ...
  const handleDownloadCertificate = async () => {
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/donors/impact-report`, {
        method: 'GET',
        headers: {
      ...
      if (!response.ok) {
        ...
        console.error('Failed to download impact report:', response.status, errorText);
        alert(`Failed to download impact certificate (${response.status})`);
        return;
      }
      ...
      link.href = url;
      link.setAttribute('download', 'ShareMeal_Impact_Certificate.pdf');
      ...
      alert('Error downloading impact certificate. Please try again.');
    } finally {
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-slate-900 text-white p-6 rounded-lg border border-slate-800">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            className="px-4 py-2.5 text-xs font-semibold text-white bg-emerald-600 hover:bg-emerald-500 rounded-md transition-colors flex items-center gap-1.5"
          >
